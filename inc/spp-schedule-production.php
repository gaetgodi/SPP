...
/* =========================================================
   ...
   - Pass 1.5 now handles all + preferences (+5:30, +6:40, +7:50)
     using a generic loop — not just +5:30.
   ...
   - Fixed missing last_name, user_phone, user_email for Phase 3
     replacement players in Schedules. Schedules is built from
     groupings JOIN before Phase 3 runs, so Phase 3 inserts
     bypass the JOIN. Added UPDATE from Master after each insert.

   Changes from 1.8.8:
   - Phase 3 now fetches first_name and last_name from Master
     when inserting candidates into Schedules -- preferred_permanent
     may have NULL last_name causing blank display.
   - Phase 3 strips P- prefix from first_name before storing
     deferred players in preferred_new.

   Changes from 1.8.7:
   - Fixed preferred_new table always empty: INSERT was omitting
     NOT NULL columns (Rank, first_name, last_name, user_phone).
     Now inserts all columns with values from Master table.

   Changes from 1.8.6:
   - Fixed "Value of type null is not callable" when Phase 3
     runs with preferred table active. Root cause: helper
     closures ($normalize_travel, $extract_carpool, $carpool_key
     etc.) were defined AFTER the preferred_new population code
     that calls them. Moved HELPER FUNCTIONS section to before
     preferred_new population.

   Changes from 1.8.5:
   - Fixed negative $extra_registrants when registrants exceed
     max_slots * 5 (e.g. 86 registrants with 5 courts = 75 max).
     $total_registrants now capped at groups_needed * 5 so
     excess players correctly go to preferred_new.

   Changes from 1.8.4:
   - Phase 3 completely rewritten -- no array key access inside
     strings, no curly brace interpolation, array() syntax
     instead of [], explicit if/else instead of ?:.

   Changes from 1.8.3:
   - Fixed double-encoded UTF-8 bytes in Phase 3 swap messages
     using exact byte replacement.

   Changes from 1.8.3:
   - Phase 3 completely rewritten to avoid all curly brace
     and array key access patterns that Code Manager
     interprets as unclosed shortcode tags.
   - All array accesses extracted to plain variables.
   - Short ternary ?: replaced with explicit if/else.
   - [] array syntax replaced with array() throughout Phase 3.

   Changes from 1.8.2:
   - Replaced UTF-8 checkmark and em dash in Phase 3 swap
     messages with plain ASCII "OK:" and "--" to avoid
     Code Manager encoding issues.

   Changes from 1.8.1:
   - Phase 3 swap messages converted from {$var} interpolation
     to string concatenation — Code Manager was choking on
     curly brace interpolation syntax.

   Changes from 1.8.0:
   - Fixed PHP 8.2 deprecated nested array access inside string
     interpolation — extracted $player_ranks[$m[key]] to variable.
   - Fixed $wpdb->prepare() with no placeholders in preferred_new
     carpool partner lookup — replaced with direct get_results().

   Changes from 1.7:
   - Phase 1 group swap messages show time labels instead of
     time IDs, and violation score removed.
   - Phase 2 player swap messages show player names and user IDs
     instead of just rank, and violation score removed.
   - $time_labels moved up before Phase 1 so both phases use it.

   Changes from 1.5:
   - Removed mid-algorithm ⚠ warning messages from Passes 1,
     1.5, 2, and 3 — these reflected intermediate states that
     post-processing may fix, causing confusion for Anne.
   - Carpool violation report replaced with plain English:
     shows player names and time slots instead of score.

   Changes from 1.4:
   - Added normalize_travel() — single function that cleans all
     travel string variants before any regex matching:
       "- 5:30"     → "-5:30"
       "-5:30Mogdan"→ "-5:30 Mogdan"
       "-7:50pm"    → "-7:50"
       "5:30"       → "+5:30"  (bare time = preference)
       "7:50"       → "+7:50"  (bare time = preference)
   - $has_travel_conflict uses normalized travel
   - $extract_carpool uses normalized travel — correctly extracts
     carpool name from "-5:30 Mogdan", "+5:30 SAE" etc.
   - Pass 1.5 uses normalized travel for +5:30 detection
   - Replaced single-pass travel conflict resolution with
     iterative group swap + iterative player swap post-processing:
       Phase 1: iterative group swaps (up to 3 passes)
       Phase 2: iterative player swaps (up to 3 passes)
     Violations fixed in priority order:
       1. -5:30 player at 5:30
       2. -7:50 player at 7:50
       3. +5:30 player not at 5:30
       4. Priority carpool partners not in same slot
       5. Regular carpool partners 2 slots apart

   Changes from 1.3:
   - Removed Schedules_Scores_{$Event} creation (moved to
     Apply Override step 5.5 — only created after results published)

   Changes from 1.2:
   - Pass 1.5: relaxed swap candidate constraint — now only
     rejects groups with -5:30 or +5:30 players, allowing
     groups with carpools or other travel notes to be used
     as swap candidates, giving more flexibility for +5:30
     placement.

   Changes from 1.1:
   - Added +5:30 priority placement (Pass 1.5) in Schedules
     table phase after groupings are built.

   Changes from 1.0:
   - Added +5:30 priority placement before -5:30 conflict resolution
   - Auto-insert missing Groups table rows when courts added
   ========================================================= */
...
if (isset($Event) and $Event <> 0) {
    ...
    // Silently remove corrupted entries with user_id = 0
    $wpdb->query("DELETE FROM {$prefix}rtec_entries WHERE user_id = 0 AND event_id = $Event");
    ...
    // Cap total_registrants to what can actually be scheduled
    // (groups_needed * 5). Excess go to preferred_new.
    ...
    // Non-selected players added to preferred_new only if their
    // carpool partner is ALSO not selected (or they have no partner).
    // This prevents splitting a carpool pair across play/defer.
    // -------------------------------------------------------
    ...
    foreach ($not_selected_users as $uid) {
        ...
        // Get carpool name for this user
        ...
        if (empty($cp_name)) {
            // No carpool — safe to add individually
            $safe_for_preferred[] = $uid;
        } else {
            // Find carpool partners among all registrants
            ...
            // Check if any partner is currently playing
            ...
            if ($partner_playing) {
                // Partner is playing — exclude this player from preferred_new
                // to avoid splitting the carpool
                $already_handled[$uid] = true;
            } else {
                // All partners also not selected — add all together
                $safe_for_preferred[] = $uid;
                foreach ($partner_ids as $pid) {
    ...
    // Sort descending — 5-player groups first, 4-player groups last.
    arsort($group_sizes);
    ...
    // -------------------------------------------------------
    // BUILD CARPOOL MAPS — priority (+) and regular
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // BASE TIME SLOT DISTRIBUTION — uneven, excludes even split
    // -------------------------------------------------------
    $valid_combos = [];
    ...
    // Priority carpools first, then regular
    // -------------------------------------------------------
        ...
        foreach ($carpools as $carpool_name => $gids) {
            ...
            foreach ($gids as $gid) {
                if ($group_time_map[$gid] != $target_time) {
                    ...
                    // ⚠ removed — post-processing may resolve this
                }
            }
        }
    };
    ...
    // Force +time groups to their preferred time slot before
    // conflict resolution. Handles +5:30, +6:40, +7:50.
    // -------------------------------------------------------
    ...
    // Each time slot index: 0=5:30, 1=6:40, 2=7:50
    $pref_patterns = array(
    ...
    foreach ($pref_patterns as $pref) {
        foreach ($players_per_group as $gid => $players) {
            ...
            foreach ($group_time_map as $other_gid => $other_tid) {
                ...
                // Check: no player in swap candidate has hard block for preferred time
                $has_conflict = false;
                ...
                // Check: swapping won't break a carpool 2+ slots apart
            ...
            // No warning — post-processing may resolve remaining violations
        }
    }

    // -------------------------------------------------------
    // CARPOOL ADJUSTMENTS — PASS 2
    // Enforce max 1 time slot apart (all carpools)
    // -------------------------------------------------------
    ...
    // Individual player moves with cascading
    // -------------------------------------------------------
    ...
    foreach ($all_carpools as $carpool_name => $gids) {
        ...
        for ($i = 0; $i < count($gids); $i++) {
            for ($j = $i + 1; $j < count($gids); $j++) {
                ...
                // ⚠ removed — final report will show any remaining issues
            }
        }
    }
    ...
    foreach ($players_per_group as $group_id => $players) {
        ...
        foreach ($players as $user_id) {
            ...
            $registration_date = $wpdb->get_var($wpdb->prepare(
                "SELECT registration_date FROM {$wpdb->prefix}rtec_entries WHERE user_id = %d AND event_id = %d AND status = 'confirmed'",
                $user_id, $event
            ));
    ...
    // Display preferred_new table (excluding permanent preferred)
    ...
    // Save pre-travel snapshot for before/after comparison
    ...
    $score_violation = function($uid, $time_id) use ($normalize_travel, $wpdb, $Master, $time_ids) {
        ...
        $t_pos = array_search($time_id, $time_ids);
        // Hard constraints -- player must NOT be at this time
        ...
        // Soft preferences -- player WANTS to be at this time
    ...
    // Time slot labels for human-readable messages
    $time_labels = [
        $time_ids[0] => '5:30pm',
        $time_ids[1] => '6:40pm',
        $time_ids[2] => '7:50pm',
    ...
    for ($pass = 1; $pass <= $max_player_passes; $pass++) {
        ...
        foreach ($violating_players as $vp) {
            ...
            foreach ($candidates as $cp) {
                ...
                $gsize2 = (int)$wpdb->get_var("SELECT COUNT(*) FROM $Schedules WHERE group_id = $gid2 AND group_id != 99");
    ...
    // -------------------------------------------------------
    // PHASE 3 — PREFERRED TABLE SWAP (last resort)
    // For players still with travel violations after Phase 1+2,
    // attempt to swap them out with preferred_new candidates.
    // -------------------------------------------------------
    ...
    // Get all players from Schedules
    ...
    foreach ($still_violating as $vp) {
        ...
        if (empty($cp_name1)) {
            ...
            // Fetch name from Master — preferred_permanent may have NULL last_name
            ...
            // Update last_name, phone, email from Master — Schedules was built before Phase 3
            ...
            // Strip P- prefix if present before storing in preferred_new
            ...
            // Carpool player -- find all partners
    ...
    // -------------------------------------------------------
    // CARPOOL REPORT — plain English with names and times
    // -------------------------------------------------------
